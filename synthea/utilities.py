...
def parse_dimensions(self, dimensions: str) -> tuple[int, int]:
    ...
    if len(values) != 2:
        raise InvalidImageDimensionsException(
            f"Couldn't parse '{dimensions}' into a width and height",
        )
    ...
    if width < 16 or height < 16:
        raise InvalidImageDimensionsException(
            f"Invalid dimensions {dimensions} - minimum size is 16x16",
        )
    if width > 16384 or height > 16384:
        raise InvalidImageDimensionsException(
            f"Invalid dimensions {dimensions} - maximum size is 16384x16384",
        )
