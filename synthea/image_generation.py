...
class ImageModel:
    ...
    def _apply_values_to_workflow(
        ...
        workflow[str(self.config.comfyui_prompt_node_id)]["inputs"][
            self.config.comfyui_prompt_input_name
        ] = positive_prompt
        workflow[str(self.config.comfyui_width_node_id)]["inputs"][
            self.config.comfyui_width_input_name
        ] = width
        workflow[str(self.config.comfyui_height_node_id)]["inputs"][
            self.config.comfyui_height_input_name
        ] = height
        workflow[str(self.config.comfyui_seed_node_id)]["inputs"][
            self.config.comfyui_seed_input_name
        ] = seed
    ...
    async def generate_image_from_prompt(
        ...
        async with self.processing_lock:
            with open(self.config.comfyui_workflow_path) as f:
                workflow = json.load(f)

            seed = random.random() * 1000000000
            self._apply_values_to_workflow(
